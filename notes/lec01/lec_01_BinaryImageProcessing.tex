...
Each element $I[i,j]$ is a \textbf{pixel} with \textbf{intensity} (gray level) in $\{0,\dots,K-1\}$, where, 0 represents black and $K-1$ represents white.
}
\vspace{5pt}
...
\dfn{Otsu’s Binarization}{The process of determining the threshold $T^*$ that maximizes the inter-class variance (or minimizes the intra-class variance) between foreground and background, thus optimally segmenting a bimodal histogram image.}
...
\noindent \textbf{Proof of Equivalence.} Since $\sigma^2 = \sigma_w^2(T) + \sigma_b^2(T)$ for all $T$, maximizing $\sigma_b^2(T)$ is the same as minimizing $\sigma_w^2(T)$, because \(\sigma^2\) is independent of \(T\). 

\noindent Now, for any two thresholds \(T_1,T_2\) ,Suppose $$\sigma_b^2(T_1) > \sigma_b^2(T_2)$$ subtracting the same constant \(\sigma^2\) from both sides gives
\[
...
% \noindent Equivalently, using the explicit form \[\sigma_b^2(T)=\omega_0(T)\,\omega_1(T)\big(\mu_0(T)-\mu_1(T)\big)^2\]
...
\subsection{Method 1: Minimization of Within-Class Variance}
...
\nt{This method uses Second order statistics and requires us to compute $\sigma_i^2$ for every t, which is inefficient. Hence we use the Method 2, maximizing the inter class variance in practice, which only depends on First order statistics and since we proved that maximizing $\sigma_b^2(T)$ is the same as minimizing $\sigma_w^2(T)$, and are dual's of each other, the $T*$ in solving both the optimization problems yields same value. Thus the Method 2 is more popular computationally.}
...
\subsection{Algorithm: Otsu’s Binarization (generalization)}
...
$$
J(i,j) = OR \{ B \diamond I(i,j) \}
$$

where
\[
OR\{x_1, x_2, \ldots, x_n\} = \bigvee_{k} x_k = x_1 + x_2 + \cdots + x_n = \text{max}(x_1, x_2, \ldots, x_n)
\]
...
$$
J(i,j) = AND \{ B \diamond I(i,j) \}
$$

where 
\[
AND\{x_1, x_2, \ldots, x_n\} = \bigwedge_{k} x_k = x_1 \cdot x_2 \cdot \ldots \cdot x_n = \text{min}(x_1, x_2, \ldots, x_n)
\] 
